...
// ====================================================================
// --- EVENTOS --- (Componente original, sem mudanças)
// ====================================================================
...
function GroupsPanel() {
    ...
    const [eJoinLink, setEJoinLink] = useState("");
    const [eDescription, setEDescription] = useState("");
    const [saving, setSaving] = useState(false);

    const [confirmEdit, setConfirmEdit] = useState<WhatsAppGroup | null>(null);
    ...
    async function load() {
        ...
        setListError(null);
        try {
            setList(await getWhatsAppGroups());
        } catch (err: any) {
            setListError(err?.response?.data?.message || "Erro ao carregar grupos.");
        } finally {
            setLoading(false);
        }
    }
    ...
    async function onCreate(e: React.FormEvent) {
        e.preventDefault();
        setCreateError(null);
        if (!name.trim() || !joinLink.trim()) {
            setCreateError("Preencha Nome e Link do Grupo.");
            return;
        }
        setCreating(true);
        try {
            const created = await createWhatsAppGroup({ name: name.trim(), joinLink: joinLink.trim(), description: description.trim() });
            setList((p) => [created, ...p]);
            setName("");
            setJoinLink("");
            setDescription("");
        } catch (err: any) {
            setCreateError(err?.response?.data?.message || "Erro ao criar grupo.");
        } finally {
            setCreating(false);
        }
    }

    function askEdit(g: WhatsAppGroup) {
        setListError(null);
        setConfirmEdit(g);
    }
    function proceedEdit() {
        ...
        setEditing(g);
        setEName(g.name ?? "");
        setEJoinLink(g.joinLink ?? "");
        setEDescription(g.description ?? "");
        setOpenEdit(true);
        setConfirmEdit(null);
    }
    ...
    async function onSaveEdit() {
        ...
        setSaving(true);
        try {
            const updated = await updateWhatsAppGroup(editing._id, { name: eName, joinLink: eJoinLink, description: eDescription });
            setList((p) => p.map((x) => (x._id === editing._id ? updated : x)));
            setOpenEdit(false);
        } catch (err: any) {
            setListError(err?.response?.data?.message || "Erro ao salvar grupo.");
            setOpenEdit(false);
        } finally {
            setSaving(false);
        }
    ...
    function askDelete(g: WhatsAppGroup) {
        setListError(null);
        setConfirmDelete(g);
    }
    ...
    async function proceedDelete() {
        ...
        setListError(null);
        try {
            await deleteWhatsAppGroup(confirmDelete._id);
            setList((p) => p.filter((x) => x._id !== confirmDelete._id));
            setConfirmDelete(null);
        } catch (err: any) {
    ...
    return (
        <>
            <Card title="Novo Grupo">
                <form onSubmit={onCreate} className="grid gap-4">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                        ...
                        <Field label="Link para Entrar (joinLink)">
                            <Input value={joinLink} onChange={(e) => setJoinLink(e.target.value)} placeholder="https://chat.whatsapp.com/..." />
                        </Field>
                    </div>
                    <Field label="Descrição (opcional)">
            ...
            <Card title="Grupos Publicados">
                {loading ? (
                    ...
                    <>
                        ...
                        <ul className="space-y-3 mt-4">
                            {list.map((g) => (
                                <li key={g._id} className="rounded-xl border bg-white p-4 flex items-start justify-between">
                                    <div className="min-w-0">
                                        <div className="font-medium truncate">{g.name}</div>
                                        <div className="text-xs text-gray-500 mt-1 truncate">{g.joinLink}</div>
                                        {g.description ? <div className="text-sm text-gray-600 mt-1 line-clamp-2">{g.description}</div> : null}
                                    </div>
                                    <div className="flex gap-2 shrink-0">
                                        <Button variant="secondary" className="rounded-xl" onClick={() => askEdit(g)}>Editar</Button>
                                        <Button variant="destructive" className="rounded-xl" onClick={() => askDelete(g)}>Excluir</Button>
                                    </div>
                                </li>
                            ))}
                        </ul>
            ...
            {/* Diálogos de Confirmação (Editar e Deletar) */}
            <Dialog open={!!confirmEdit} onOpenChange={(v) => !v && setConfirmEdit(null)}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Editar este grupo?</DialogTitle></DialogHeader>
                    <div className="mt-4 flex justify-end gap-2">
                        <DialogClose asChild><Button variant="secondary" className="rounded-lg">Cancelar</Button></DialogClose>
                        <Button className="rounded-lg" onClick={proceedEdit}>Continuar</Button>
                    </div>
                </DialogContent>
            </Dialog>
            <Dialog open={!!confirmDelete} onOpenChange={(v) => !v && setConfirmDelete(null)}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Excluir este grupo?</DialogTitle><DialogDescription>Esta ação não pode ser desfeita.</DialogDescription></DialogHeader>
                    <div className="mt-4 flex justify-end gap-2">
                        <DialogClose asChild><Button variant="secondary" className="rounded-lg">Cancelar</Button></DialogClose>
                        <Button variant="destructive" className="rounded-lg" onClick={proceedDelete}>Excluir</Button>
                    </div>
            ...
            <Dialog open={openEdit} onOpenChange={setOpenEdit}>
                <DialogContent>
                    ...
                    <div className="grid gap-4">
                        <Field label="Nome do Grupo"><Input value={eName} onChange={(e) => setEName(e.target.value)} /></Field>
                        <Field label="Link para Entrar"><Input value={eJoinLink} onChange={(e) => setEJoinLink(e.target.value)} /></Field>
                        <Field label="Descrição"><Textarea rows={3} value={eDescription} onChange={(e) => setEDescription(e.target.value)} /></Field>
                        <div className="flex items-center gap-2">
                            <Button className="ml-auto rounded-xl" onClick={onSaveEdit} disabled={saving}>{saving ? "Salvando..." : "Salvar"}</Button>
                        </div>
                    </div>
                </DialogContent>
            </Dialog>

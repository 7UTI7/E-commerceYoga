...
export default function VerificacaoCadastro() {
    ...
    // useRef mantém o valor entre renderizações mas não causa re-render
    // Usamos isso para impedir que o React chame a API duas vezes (Strict Mode)
    ...
    useEffect(() => {
        ...
        async function executeVerification() {
            ...
            // Marca que já rodou ANTES da chamada assíncrona
            ...
            try {
                ...
                // Se quiser salvar o token, pode fazer aqui
                if (data.token) {
                    localStorage.setItem("auth_token", data.token);
                }

            } catch (err: any) {
                // Só mostramos erro se a PRIMEIRA tentativa falhar.
                // Como o useRef bloqueia a segunda, esse erro aqui é real.
                setSuccess(false);
        ...
        // Cleanup function (opcional, boa prática)
        return () => {
            effectRan.current = true;
        };
    }, [token]);

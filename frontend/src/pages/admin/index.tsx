...
function ClassesPanel() {
  ...
  const [lvl, setLvl] = useState<Level>("Todos");
  const [creating, setCreating] = useState(false);
  ...
  async function onCreate(e: React.FormEvent) {
    ...
    setCreating(true);
    try {
      const created = await createClassSlot({ title, description: desc, dateTime: toISO(dt), durationMinutes: Number(dur), maxStudents: Number(max), level: lvl } as any);
      setList(p => [created, ...p]);
      setTitle(""); setDt(""); setDesc(""); setDur(""); setMax("");
      showFeedback('success', "Aula agendada!");
    } catch { showFeedback('error', "Erro ao agendar aula."); }
    finally { setCreating(false); }
  }
...
function GroupsPanel() {
  ...
  async function onCreate(e: React.FormEvent) {
    e.preventDefault();
    setFeedback(null);
    if (!name || !link) return showFeedback('error', "Preencha os campos obrigatórios.");
    setCreating(true);
    try {
      const created = await createWhatsAppGroup({ name, joinLink: link, description: desc });
      setList(p => [created, ...p]);
      setName(""); setLink(""); setDesc("");
      showFeedback('success', "Grupo criado!");
    } catch { showFeedback('error', "Erro ao criar grupo."); }
    finally { setCreating(false); }
  }
